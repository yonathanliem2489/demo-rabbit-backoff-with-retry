...
@Slf4j
public class RetryQueuesDelayedInterceptor implements MethodInterceptor {
    ...
    public RetryQueuesDelayedInterceptor(RabbitTemplate rabbitTemplate, RetryQueues retryQueues,
    ...
    private void sendToNextRetryQueue(MessageAndChannel mac, int retryCount) throws Exception {
        rabbitTemplate.convertAndSend(BindingConfiguration.EXCHANGE_DELAYED_NAME,
            BindingConfiguration.ROUTING_KEY_DELAYED_NAME, mac.message, m -> {
    ...
    private void publishToParkingLotQueue(String problem,
        ...
        try {
            rabbitTemplate.convertAndSend(BindingConfiguration.EXCHANGE_DELAYED_NAME,
                BindingConfiguration.DELAYED_PARKING_KEY, ParkingLot.builder()
                    .problem(problem)
